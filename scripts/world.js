...
function mountainNoise(wx, wz) {
    ...
    const maskT = Math.max(0, (maskRaw - 0.54) / 0.26);
...
export class VoxelWorld {
        ...
        if (this.dimension === DIM_MOON) {
            ...
            for (let lx = 0; lx < CHUNK; lx++) {
                ...
                for (let ly = 0; ly < CHUNK; ly++) {
                    const wy = y0 + ly;
                    if (wy <= surf) data[lx + CHUNK*(ly + CHUNK*lz)] = MOONSTONE;
                }
            }
    ...
    _ensureColLit(cx, cz) {
        ...
        for (let lx = 0; lx < CHUNK; lx++) {
            for (let wy = MAX_Y-1; wy >= MIN_Y; wy--) {
                this._setLight(wx0+lx, wy, wz0+lz, MAX_LIGHT);
    ...
    updateLightAt(mx, my, mz) {
        ...
        let startLight = 0;
        if (this.get(mx, my, mz) === MOLTENROCK) startLight = MAX_LIGHT;
        ...
        if (!this.isSolid(mx, my+1, mz) && my+1 >= MAX_Y) startLight = MAX_LIGHT;
        if (startLight === 0 && !this.isSolid(mx, my, mz)) {
            // Might be at the very top of a newly opened shaft — do a vertical scan
            this.litCols.delete(`${Math.floor(mx/CHUNK)},${Math.floor(mz/CHUNK)}`);
    ...
    _propagateLight(q) {
        ...
        while (head < q.length) {
            ...
            const lv = this.getLight(x, y, z);
            if (lv <= 1) continue;
            for (let d = 0; d < 18; d += 3) {
                const nx = x+LDIRS[d], ny = y+LDIRS[d+1], nz = z+LDIRS[d+2];
                if (ny < MIN_Y || ny >= MAX_Y) continue;
                if (this.isSolid(nx, ny, nz) && this.get(nx, ny, nz) !== MOLTENROCK) continue;
                if (this.getLight(nx, ny, nz) >= lv-1) continue;
