...
import { DIM_MOON, WOODPLANKS } from './world.js';
...
// Pre-computed 3×3×3 Gaussian weights — must match mesher.js exactly
...
function density(world, x, y, z) {
    ...
    // WOODPLANKS: direct containment check before any interpolation.
    ...
    if (world.get(ix, iy, iz) === WOODPLANKS) return 1;
    ...
    const s = _voxelMode
        ...
        : (vx, vy, vz) => {
            // Treat plank corners as 0 so they don't bleed density into adjacent
            // smooth terrain — plank collision is handled by the containment check.
            if (world.get(vx, vy, vz) === WOODPLANKS) return 0;
...
export class Player {
    ...
    update(dt, input, isFlying = false, inceptionMode = false, speedMultiplier = 1) {
        ...
        const speed = (isRunning ? RUN_SPEED : MOVE_SPEED) * speedMultiplier;
        ...
        this.mesh.position.set(this.pos.x, this.pos.y + PLAYER_H*0.5, this.pos.z);
    }
    ...
    getEyePosition() {
        ...
        return new THREE.Vector3(this.pos.x + bobX, this.pos.y + EYE_HEIGHT + bobY, this.pos.z);
    }
